...
if [ "$PLATFORM" == "windows" ]; then
  scons platform=windows arch=${ARCH} use_mingw=yes ${SCONS_FLAGS} target=template_release
  scons platform=windows arch=${ARCH} use_mingw=yes ${SCONS_FLAGS} target=template_debug
  
  mv bin/godot.windows.template_release.${ARCH}.exe $BIN_DIR/windows_release_${ARCH}.exe
  mv bin/godot.windows.template_release.${ARCH}.console.exe $BIN_DIR/windows_release_${ARCH}_console.exe
  mv bin/godot.windows.template_debug.${ARCH}.exe $BIN_DIR/windows_debug_${ARCH}.exe
  mv bin/godot.windows.template_debug.${ARCH}.console.exe $BIN_DIR/windows_debug_${ARCH}_console.exe
  ...
  scons platform=linuxbsd arch=${ARCH} ${SCONS_FLAGS} target=template_release
  scons platform=linuxbsd arch=${ARCH} ${SCONS_FLAGS} target=template_debug
  
  mv bin/godot.linuxbsd.template_release.${ARCH} $BIN_DIR/linux_release.${ARCH}
  mv bin/godot.linuxbsd.template_debug.${ARCH} $BIN_DIR/linux_debug.${ARCH}
